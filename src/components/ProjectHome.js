...
function EditableName({ name, onSave }) {
  ...
  return (
    <h1 className="proj-home-name" onClick={() => setEditing(true)} title="Click to rename">
      {name}
      <span className="proj-home-name-pencil">{PENCIL_SVG}</span>
    </h1>
  );
}
...
function ResumeCard({ resume, onEdit, onDelete }) {
  ...
  return (
    <div className="proj-doc-card resume-card">
      ...
      {/* Thumbnail — only click target to open editor */}
      {template && (
        <div className="proj-doc-thumbnail proj-doc-thumbnail-clickable" onClick={onEdit}>
          <TemplateSVG template={template} />
          <div className="proj-doc-thumbnail-overlay">
            {PENCIL_SVG}
            Edit Resume
          </div>
        </div>
      ...
      {/* Info — delete button sits on the same row as counts */}
      <div className="proj-doc-info">
        ...
        <div className="proj-doc-counts">
          <div className="proj-doc-counts-text">
    ...
    <div className="proj-doc-card cl-card">
      ...
      {cls.length === 0 ? (
        <div className="proj-doc-empty-state">
          ...
          {!hasResume && (
            <p className="proj-doc-tip">💡 Add a resume first so the AI has more to work with.</p>
          )}
        ...
        <div className="proj-cl-list">
          {cls.map(cl => (
            <div key={cl.id} className="proj-cl-item">
              ...
              {confirmId === cl.id ? (
                ...
                <div className="proj-cl-item-actions">
                  <button className="btn-proj-cl-edit-icon" onClick={() => onEditCoverLetter(cl.id)} title="Edit cover letter">
                    {PENCIL_SVG}
                    Edit Cover Letter
                  </button>
